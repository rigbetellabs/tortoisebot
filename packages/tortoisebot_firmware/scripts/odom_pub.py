...
class odometry_publisher:
    ...
    def get_init_pose(self):
        while True:
            try:
                old_pose = self.listener.lookupTransform(
    ...
    def calc_velocity(self, new_pose):
        ...
        o.twist.twist.linear.x = (
            new_pose[0][0] - self.old_pose[0][0]
        ) / self.sleep_freq
        o.twist.twist.linear.y = (
            new_pose[0][1] - self.old_pose[0][1]
        ) / self.sleep_freq
        o.twist.twist.linear.z = (
            new_pose[0][2] - self.old_pose[0][2]
        ) / self.sleep_freq
        o.twist.twist.angular.x = (
            new_pose[1][0] - self.old_pose[1][0]
        ) / self.sleep_freq
        o.twist.twist.angular.y = (
            new_pose[1][1] - self.old_pose[1][1]
        ) / self.sleep_freq
        o.twist.twist.angular.z = (
            new_pose[1][2] - self.old_pose[1][2]
        ) / self.sleep_freq
    ...
    def run(self):
        while not rospy.is_shutdown():
            try:
                ...
                new_pose = self.listener.lookupTransform(
                    "/odom", "/base_link", rospy.Time(0)
                )
